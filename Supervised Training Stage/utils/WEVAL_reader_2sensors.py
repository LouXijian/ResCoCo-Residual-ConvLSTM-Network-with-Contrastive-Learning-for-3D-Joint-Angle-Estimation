...
def rotate_augmentation(imu, mocap, augmentation_size, align_range):
    ...
    for j in range(augmentation_size-1):
        ...
        imu = torch.tensor(new_imu)
        mocap = original_mocap
    return imu, mocap
...
def extract_data(datapath, seq_len, interval, set_type, label_len):
    ...
    subject_division = {'training': [1, 12], 'validation': [13, 13], 'testing': [15, 15]}
    ...
    for i in range(start_subject, end_subject + 1):
        ...
        for j in range(10):
            if i ==14:
                ...
                # imu_pos = [random.choice(shank_sensor_pos), random.choice(['DisThShim', 'ProxThShim'])]
            else:
                # imu_pos = [random.choice(shank_sensor_pos), random.choice(thigh_sensor_pos)]
                imu_pos = [random.choice(thigh_sensor_pos), random.choice(shank_sensor_pos)]
            for imu in imu_pos:
...
def input_label_loader(datapath, seq_len, interval, norm_data, set_type, rotate_aug_size, placement_aug_size,
                       batch_size, label_len, align_range):
    imu, mocap = [], []
...
def WEVAL_processor(datapath, seq_len, interval, batch_size, label_len, rotate_aug_size, placement_aug_size,
                    align_range):
    train_loader, norm_data = input_label_loader(datapath, seq_len, interval, None, 'training', rotate_aug_size,
                                                 placement_aug_size, batch_size, label_len, align_range)
    val_loader, _ = input_label_loader(datapath, seq_len, interval, norm_data, 'validation', 1,
                                       1, batch_size, label_len, align_range)
    test_loader, _ = input_label_loader(datapath, seq_len, interval, norm_data, 'testing', 1,
                                        1, batch_size, label_len, align_range)
